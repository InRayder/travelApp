...
### AI 與功能優化 (AI & Enhancements)
- **AI 回應能力升級**: 在 AI 助手、對話翻譯、Vlog 腳本、行程建議與活動編輯等流程導入 `googleSearch` 工具，提升即時查詢與建議品質。
- **AI Service 調整**: 將攻略生成與行程建議的模型工具設定統一為 `googleSearch`，減少工具配置不一致問題。

### 使用體驗改善 (UX Improvements)
- **攻略匯入更聰明**: 匯入攻略時可自動從貼文內容擷取 URL，若網址混在文字中會自動清理並填入連結欄位，降低手動整理成本。

### 文件與開發支援 (Docs & DX)
- **README 重整**: 更新專案介紹、功能列表、技術棧與目錄說明，補齊 AI、PWA 與開發流程資訊。
- **測試工具依賴**: 新增 `puppeteer-core` 開發依賴，支援自動化檢查與腳本測試場景。
